...
def get_expert(mol):
    ...
    if edges_sub_len == len(edges):
        ...
        graph_sub:nx.Graph = nx.Graph(graph.subgraph(max(nx.connected_components(nx.Graph(edges_sub)),key=len)))
        for edge in graph_sub.edges():  #when a ring is broken, the sub connected graph will also include the bond and atom, remove it
            if (edge[0],edge[1]) not in edges_sub and (edge[1],edge[0]) not in edges_sub:
                graph_sub.remove_edges_from([edge])
        possible_edges = compare_graph(graph,graph_sub)
        assert len(possible_edges) > 0
        graph_nodes,sub_nodes = list(graph),list(graph_sub)
        for (node_1,node_2) in possible_edges:
            bond_type = POSSIBLE_BONDS.index(graph.edges()[node_1,node_2]['bond_type'])
            if node_1 in sub_nodes and node_2 in sub_nodes: # a ring is formed
                ...
                node_2_symbol = graph.nodes()[node_2]['symbol']
                ...
                node_1_symbol = graph.nodes()[node_1]['symbol']
...
def compare_graph(graph:nx.Graph,graph_sub:nx.Graph):
    graph_edges,sub_edges = list(graph.edges()),list(graph_sub.edges())
    sub_nodes = list(graph_sub)
    posible_edges = []
    for (start,end) in graph_edges:
        if (start,end) in sub_edges or (end,start) in sub_edges:
            continue
        else:
...
class ExpertDataset(Dataset):
    ...
    def tensorize(self,smi_batch):
        filtered_mol_batch,nx_graphs,expert_actions = [],[],[]
        ...
        for mol in filtered_mol_batch:
            nx_graph,action = get_expert(mol)
            nx_graphs.append(nx_graph)
            expert_actions.append(action)
        
        mol_features = tensorize_nxgraphs(nx_graphs)
        return mol_features,expert_actions

class TrajectoryState():
    def __init__(self,nx_graph,action:torch.LongTensor,step_reward:float):
        self.nx_graph = nx_graph
        self.action = action 
        self.reward = step_reward
        #self.credicted_reward = None
        self.probs = None
...
class StatesGenerator():
    def __init__(self,processed_dir,batch_size:int,pool_size=5000):
...
class StatesDataset(IterableDataset):
    ...
    def tensorize_state(self,state:TrajectoryState):
        (fatoms,adj_matrix,all_mask,current_mask) = tensorize_single_nxgraph(state.nx_graph)
        #reward = torch.Tensor([state.credicted_reward])
        action = state.action.long().squeeze()
        probs = state.probs.squeeze()
        v_target = torch.Tensor([state.v_target])
        advantage = torch.Tensor([state.advantage])
        advantage = torch.Tensor([(state.advantage - self.adv_mean) / (self.adv_std + 1e-7)])
        return fatoms,adj_matrix,all_mask,current_mask,action,probs,v_target,advantage
