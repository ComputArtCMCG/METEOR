...
from states import StatesDataset,StatesGenerator,RawTrajectoryProcessor,SAScoreEstimator,PlanetEstimator,ScaffoldMemory
from states import ActivityDataset,ActivityStatWithDataset
...
class PPO():
    ...
    def synchronize(self):
        self.actor_critic.load_state_dict(torch.load(os.path.join(self.work_dir,'actorcritic_round{:03d}.param'.format(self.round)),map_location=self.device))
    ...
    def training_loop(self,dataloader:DataLoader):
        ...
        for _ in tqdm(range(self.training_epoch)):
            for (fatoms,adj_matrix,all_mask,current_mask,action_batch,probs_batch,v_target_batch,advantage_batch) in dataloader:
                optimizer.zero_grad()
                ...
                loss,critic_l,entropy,approxy_kl = self.actor_critic(current_mol_features,action_batch,probs_batch,v_target_batch,advantage_batch,self.round)
                loss.backward()
                if self.round > 1:
