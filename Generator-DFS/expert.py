...
class ExpertSolution():
    ...
    def training_loop(self,dataloader:DataLoader):
        for mol_features,focus,prob_mask,expert_actions in dataloader:
            #try:
                ...
                fatoms_input = self.actor_critic.actor_feature_update(*mol_features)
                loss = self.actor_critic.expert_training(fatoms_input,focus,prob_mask,expert_actions)
                loss.backward()
    ...
    def expert_training(self,yaml_file):
        ...
        for _ in range(parameters['epoch']):
            ...
            dataloader = DataLoader(expert_dataset,batch_size=1,shuffle=False,num_workers=16,collate_fn=lambda x:x[0],pin_memory=True)
