...
class SCScorer():
    ...
    def restore(self, weight_path=os.path.join(os.path.dirname(__file__),'model.ckpt-10654.as_numpy.json.gz'), FP_rad=FP_rad, FP_len=FP_len):
        self.FP_len = FP_len; self.FP_rad = FP_rad
        self._load_vars(weight_path)
        #print('Scscore model has successfully restored variables from {}'.format(weight_path))
